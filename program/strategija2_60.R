...
source("program/strategija2_rezultati1_60.r")
source("program/strategija2_verjetnosti_profesionalec60.r")
source("program/strategija2_verjetnosti_zacetnik60.r")
source("program/strategija2_verjetnosti_rekreativec60.r")
source("program/strategija2_iteracija60.r")
...
#        "enolicna.stanja60"),file = "program/strategija2_rezultati1_60.r")
...
# verjetnosti.zacetnik60 <- tabela.verjetnosti(enolicna.stanja60, prehodne.verjetnosti.zacetnik, mozni.rezultati,
#                                              prehodna.matrika.tocka.1.zacetnik60)

# dump(c("verjetnosti.profesionalec60"), file = "program/strategija2_verjetnosti_profesionalec60.r")
# dump(c("verjetnosti.rekreativec60"), file = "program/strategija2_verjetnosti_zacetnik60.r")
# dump(c("verjetnosti.zacetnik60"), file = "program/strategija2_verjetnosti_rekreativec60.r")
...
izracunV <- function(vektorC, stariV, verjetnosti){
  st.tock.p <- 83
  minimum <- Matrix(data=100,nrow=3564,ncol=1)
...
#        "koncniV.zacetnik60"), file = "program/strategija2_iteracija60.r")


###### Ciljna točka za minimizacijo števila rund ######

index.ciljna.tocka <- function(vektorC, koncniV, verjetnosti){
  # funckija vrne optimalno točko p za ciljanje
  st.tock.p <- 83
  minimum <- Matrix(data=100,nrow=3564,ncol=1)
  p <- Matrix(data=100,nrow=3564,ncol=1)
  for (i in 0:(st.tock.p-1)){
    index1 <- 3564*i + 1 
    index2 <- 3564*(i+1)
    prehodna.matrika.tocka <- verjetnosti[index1:index2,]
