...
rezultat.meta2 <- function(koordinataX, koordinataY){
  # enako kot funckija rezultat.meta, le da vrne character, kjer je vključen tudi pas
  kot.polja <- 2*pi/20 # 20 razdelkov na tabli, kot v stopinjah
  tocke <- c(20,  1, 18, 4, 13, 6, 10, 15, 2, 17, 3, 19, 7, 16, 8, 11, 14, 9, 12, 5)
  koti <- seq(0, 2 * pi - kot.polja, by = kot.polja)
  koti <- koti -  0.5 * pi / 10
  ...
  pas.tocke <- c(50, 25, 1, 3, 1, 2, 0)
  tocke.meta <- if (pas <= 2) { sprintf("%s-BULL",pas.tocke[pas]) } else { sprintf("%s-%s",tocke[izbira.kota] * pas.tocke[pas], pas.tocke[pas])}    
  return(tocke.meta)
}
